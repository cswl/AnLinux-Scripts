...
if [ "$first" != 1 ];then
	if [ ! -f $tarball ]; then
		...
		*)
		...
		wget "https://raw.githubusercontent.com/EXALAB/LinuxOnAndroid/master/Rootfs/openSUSE/Tumbleweed/${archurl}/openSUSE-Tumbleweed-rootfs-${archurl}.tar.xz" -O $tarball
	fi
	cur=`pwd`
	mkdir -p "$folder"
	cd "$folder"
