...
if [ "$1" = "i386" ] || [ "$1" = "amd64" ] ; then
  debootstrap --no-check-gpg --arch=$1 --variant=minbase --include=systemd,systemd-shim,libsystemd0,wget,ca-certificates stretch $1 http://deb.debian.org/debian
else
  qemu-debootstrap --no-check-gpg --arch=$1 --variant=minbase --include=systemd,systemd-shim,libsystemd0,wget,ca-certificates stretch $1 http://deb.debian.org/debian
fi
